...
export const MODELOS = ['gemini-3.5-flash', 'gemini-3.5-flash-lite', 'gemini-3.6-flash'];
export const EXTRACOES_PARCIAIS_AUTORIZADAS = Object.freeze([
    '2026_6257_BR_280002542548',
    '2026_6257_BR_280002539826',
]);
const extracoesParciaisAutorizadas = new Set(EXTRACOES_PARCIAIS_AUTORIZADAS);
...
function extracaoParcialUtilizavel(doc) {
    if (!hashValido(doc.sha256) || doc.statusExtracao !== 'texto_parcial' || !Array.isArray(doc.paginasPendentes)
        || !doc.paginasPendentes.length || !Array.isArray(doc.paginas) || !Number.isInteger(doc.totalPaginas)
        || doc.totalPaginas < 1 || doc.paginas.length !== doc.totalPaginas) return false;
    const numeros = new Set(doc.paginas.map(p => p?.pagina));
    return numeros.size === doc.totalPaginas
        && doc.paginas.every(p => Number.isInteger(p?.pagina) && p.pagina >= 1 && p.pagina <= doc.totalPaginas
            && typeof p.texto === 'string' && typeof p.revisar === 'boolean')
        && doc.paginas.some(p => p.texto.trim());
}
...
export function prepararContexto(fonte, chave, selecao) {
    ...
    const extracaoParcialAutorizada = extracoesParciaisAutorizadas.has(chave)
        && estados.every((estado, i) => estado === 'apto' || (estado === 'extracao_pendente' && extracaoParcialUtilizavel(selecionados[i])));
    if (estados.some(e => e !== 'apto') && !extracaoParcialAutorizada) return { resposta: { ...base, estado: estados.includes('extracao_pendente') ? 'extracao_pendente' : 'revisao_necessaria', motivo: 'Cobertura ou controles de extração insuficientes.' } };
    if (extracaoParcialAutorizada) base.extracaoParcialAutorizada = true;
    ...
    return { base, chave, fonte, docs: selecionados, inventario: docs, paginas,
        avisoRevisaoOCR: extracaoParcialAutorizada || selecionados.some(d => d.requerRevisaoOCR || quantidade(d.paginasParaRevisao) > 0 || d.paginas.some(p => p.revisar || p.status === 'ocr_revisar')) };
}
